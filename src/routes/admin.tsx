...
import {
  ArrowLeft, Save, RotateCcw, Plus, Trash2, Download, Upload, Eye, Check, LogOut, KeyRound,
} from "lucide-react";
...
type TabKey =
  ...
  | "process" | "testimoni" | "faq" | "cta" | "kontak" | "footer" | "akun";
      ...
      <div className="card">
        ...
        {p.items.map((it, i) => (
          ...
          <div className="grid-2 mt-3">
              ...
              <TextField label="Harga Normal (Coret) — opsional" value={it.priceOriginal ?? ""} onChange={(v) => { const n = [...p.items]; n[i] = { ...n[i], priceOriginal: v }; set({ items: n }); }} />
            </div>
             <div className="mt-3">
                ...
                {["", "New", "Populer", "Hot", "Best"].map((opt) => {
                  ...
                  return (
                    <button
                      key={opt || "none"}
                      type="button"
                      onClick={() => { const n = [...p.items]; n[i] = { ...n[i], badge: opt }; set({ items: n }); }}
              ...
              <input
                ...
                value={it.badge ?? ""}
                onChange={(e) => { const n = [...p.items]; n[i] = { ...n[i], badge: e.target.value }; set({ items: n }); }}
              />
            </div>
          </div>
        ))}
...
function AkunTab({ email }: { email: string | null }) {
  ...
  return (
    <div className="card max-w-xl">
      <h2 className="card-title flex items-center gap-2"><KeyRound size={18} /> Ganti Password</h2>
      <p className="card-sub">
        {email ? <>Akun: <strong>{email}</strong></> : "Ubah password akun admin."}
      </p>

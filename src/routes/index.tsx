    ...
    <section id="portfolio" className="relative py-24 sm:py-32">
      ...
      <div className="relative mx-auto max-w-7xl px-5 sm:px-8">
        ...
        <div className="mt-10 flex flex-wrap justify-center gap-2.5">
          {p.cats.map((c) => (
            <button key={c} onClick={() => { setActive(c); setPage(0); }} className={`rounded-full px-5 py-2 text-xs font-semibold tracking-wide transition-all ${active === c ? "bg-primary text-primary-foreground" : "bg-card text-foreground/70 hover:bg-secondary/60"}`}>
              {c}
            </button>
          ))}
        </div>
                  ...
                  <div className="relative overflow-hidden rounded-md bg-secondary/30 aspect-[3/4]">
                    ...
                    {it.price && (
                      <div className="absolute bottom-3 left-3 flex items-center gap-2 bg-primary backdrop-blur-sm border border-primary/25 text-white px-3 py-1.5 rounded-full shadow-[0_6px_14px_-6px_rgba(0,0,0,0.25)]">
                        {it.priceOriginal && (
                          <span className="text-[10px] font-medium text-white line-through tracking-wide">{it.priceOriginal}</span>
                        )}
  ...
  return (
    <section id="testimoni" className="relative py-24 sm:py-32">
      ...
      <div className="relative mx-auto max-w-7xl px-5 sm:px-8">
        ...
        {t.stats.map((s) => {
          const Icon =
            (Icons[s.icon as keyof typeof Icons] as LucideIcon) ?? Icons.Star;
